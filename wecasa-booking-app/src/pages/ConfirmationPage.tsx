  ...
  return (
    <div className="p-6 max-w-2xl mx-auto text-center">
      ...
      <div className="mt-6 p-4 border rounded">
        <div>
          <strong>Adresse :</strong> {state.address}
        </div>
        <div>
          <strong>Rendez-vous :</strong>{' '}
          {state.appointment ? new Date(state.appointment).toLocaleString() : '—'}
        </div>
        <div>
          <strong>Prix :</strong> {formatPrice(totalPrice)}
        </div>
        <div>
          <strong>Durée :</strong> {formatDuration(totalDuration)}
        </div>
      </div>
